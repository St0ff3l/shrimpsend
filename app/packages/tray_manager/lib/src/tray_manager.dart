...
class TrayManager {
  ...
  /// macOS：展开菜单栏托盘上的截图 Popover（全屏/区域截图入口）。其他平台可能未实现。
  Future<bool> showTrayScreenshotPopover() async {
    final dynamic ok = await _channel.invokeMethod('showTrayScreenshotPopover');
    return ok == true;
  }

  /// macOS：立即开始全屏截图并进入原生标注（不经 Popover）。
  Future<bool> startFullScreenCapture() async {
    final dynamic ok = await _channel.invokeMethod('startFullScreenCapture');
    return ok == true;
  }

  /// macOS：立即开始区域截图并进入原生标注（不经 Popover）。
  Future<bool> startRegionCapture() async {
    final dynamic ok = await _channel.invokeMethod('startRegionCapture');
    return ok == true;
  }
}
