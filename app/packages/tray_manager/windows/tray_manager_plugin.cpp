...
class TrayManagerPlugin : public flutter::Plugin {
 ...
 private:
  ...
  void TrayManagerPlugin::_ShowTrayContextMenuAtCursor();
...
void TrayManagerPlugin::_ShowTrayContextMenuAtCursor() {
  ...
  TrackPopupMenu(hMenu, TPM_BOTTOMALIGN | TPM_LEFTALIGN,
                 ...
                 static_cast<int>(cursor_pos.y), 0, hWnd, NULL);
}
...
std::optional<LRESULT> TrayManagerPlugin::HandleWindowProc(HWND hWnd,
  ...
  if (message == WM_DESTROY) {
    ...
    switch (lParam) {
      case WM_LBUTTONUP:
        channel->InvokeMethod("onTrayIconMouseUp",
                              std::make_unique<flutter::EncodableValue>());
        result.emplace(static_cast<LRESULT>(0));
        break;
      case WM_RBUTTONUP:
        _ShowTrayContextMenuAtCursor();
