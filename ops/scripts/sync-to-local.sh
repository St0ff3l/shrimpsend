...
# 用法:
#   ./ops/scripts/sync-to-local.sh              # 同步配置 + 建库/迁移
#   ./ops/scripts/sync-to-local.sh --skip-db    # 仅同步配置
#   ULTRASEND_OPS_DIR=../shrimpsend-ops ./ops/scripts/sync-to-local.sh
set -euo pipefail
...
if [ ! -d "$LOCAL_DIR" ]; then
  ...
  echo "请在私有 ops 仓创建 ops/local/（见 ops/README.md）" >&2
  exit 1
fi
